...
function upload_video_to_google_drive($post_id)
{
    ...
    $client->setAuthConfig(get_template_directory_uri(). '/assets/credentials.json');
    ...
    update_post_meta($post_id, 'google_drive_video_url', $drive_url);

    // Cập nhật bài viết với shortcode
    $content = $attachment->post_content;
    $new_content = "[video src='$drive_url' width='640' height='360']";
    wp_update_post(array('ID' => $post_id, 'post_content' => $new_content));
}
add_action('add_attachment', 'upload_video_to_google_drive');
